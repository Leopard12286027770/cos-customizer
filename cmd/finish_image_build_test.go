...
func setupFinishBuildFiles() (string, *fs.Files, error) {
	...
	if err := config.Update(buildConfigFile, &config.Build{GCSBucket: "b", GCSDir: "d"}); err != nil {
...
func TestValidateFailure(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			name:      "SmallDiskSize",
			flags:     []string{"-project=p", "-zone=z", "-image-name=out", "-image-project=p", "-image-family=f", "-disk-size-gb=11", "-oem-size=1025M"},
			expectErr: true,
